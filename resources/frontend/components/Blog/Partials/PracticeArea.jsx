...
export default function PracticeArea ({closeModal, lesson}) {
    ...
    const sizes = [3, 5, 7, 10, 12];

    useEffect(() => {
        (async () => {
            let data = await getData(
                `user/practice-histories?lesson_id=${lesson.id}`
            );
    ...
    const draw = (e) => {
        ...
        const distance = Math.sqrt(
            Math.pow(offsetX - lastPosition.x, 2) +
                Math.pow(offsetY - lastPosition.y, 2)
        );
        const speed = distance / timeDiff;
        let newLineWidth = Math.max(0.1, penSize / (speed + 0.2));

        context.lineWidth =
            lastLineWidth + (newLineWidth - lastLineWidth) * 0.2;
    ...
    const handleUpdateHistory = (history) => {
        if (
            history.id != updateHistory &&
            confirm("Are you sure you want to update the history")
        ) {
    ...
    const handleSelectSampleImage = (imageurl) => {
        if (
            ...
            confirm("Are you sure you want to change the sample image")
        ) {
    ...
    const handleRemovePractice = async (history) => {
        if (confirm("Are you sure you want to remove this practice history")) {
            await postData(`user/remove-practice-history`, {
                    ...
                    <div className="col-md-4">
                        <div className="widget-sidber py-3">
                            <div className="widget-category">
                                <h3 className="blog-details-title mt-3">
                                    Samples:
                                </h3>
                                <div className="previewSamples">
                                    {lesson?.lesson_samples?.map(
                                        (sample, index) => (
                                            <img
                                                key={index}
                                                className={
                ...
                <div className="row">
                    <div className="col-lg-4">
                        <div className="submit-button">
                            <button type="button" onClick={postCanvasImage}>
